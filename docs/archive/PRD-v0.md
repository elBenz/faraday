...
17. As a distracted Mac user, I want calibration to measure desk, transition point, and my chosen phone location RSSI, so that near/far thresholds are based on real environment data.
...
- Calibration UX:
  ...
  - Optional guided calibration measures phone on desk, phone at a transition point, and phone at the user's chosen phone location.
  - The chosen phone location can be another room or a farther-away spot in the same room if RSSI separation is sufficient.
  - Calibration produces near/far thresholds and confidence notes.
- Deep modules to build:
  - Terminal UI: provides session dashboard first, then setup/calibration flows; it controls the daemon but does not own enforcement.
  ...
  - Calibration engine: records guided RSSI samples for desk, transition point, and chosen phone location, then derives thresholds.
...
- Calibration should be tested with representative sample sets for desk, transition point, and chosen phone location.
...
- Before real beacon hardware arrives, use a simulated observation source to exercise the daemon, TUI, state machine, persistence, and dry-run enforcement paths. Simulation is not accepted as final working-MVP evidence.
